...
namespace SalesWebMvcProject.Controllers
{
    public class SellersController : Controller
    {
        ...
        public async Task<IActionResult> Delete(int? id)
        {
            if (id == null)
            {
                return RedirectToAction("Error", "Sellers", new { message = "Id not provided" });
            }
            var obj = await _sellerService.FindByIdAsync(id.Value);
            if (obj == null)
            {
                return RedirectToAction("Error", "Sellers", new { message = "Seller not found" });
            }
            return View(obj);
        }
        ...
        public async Task<IActionResult> DeleteConfirmed(int id)
        {
            ...
            {
                return RedirectToAction("Error", "Sellers", new { message = e.Message });
            }
        }
        ...
        public async Task<IActionResult> Edit(int id, Seller seller)
        {
            if (id != seller.Id)
            {
                return RedirectToAction("Error", "Sellers", new { message = "Id mismatch" });
            }
            try
            {
